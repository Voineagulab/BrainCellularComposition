...
## Generic
rm(list = ls())
setwd("/Volumes/Data1/PROJECTS/BrainCellularComposition/Results/Revisions/GTEx_revised/")
options(stringsAsFactors = FALSE)


## Load in GTEx data
load("../../../Data/Preprocessed/GTEx.rda")
...
## Parameters
  ...
  ctx <- which(gtexMeta$BroadRegion == "Cortex")
...
## Filtering variables
  ...
  subneuron.colours <- pal_lancet()(6)
  names(subneuron.colours) <- signature.annotation[c(1:5, 10)]
...
## Apply to all samples
  ...
  # for (j in names(sigsBrain)[grep("z", names(sigsBrain))]) {
    for (j in names(sigsBrain)) {
      est$DRS[[j]] <- run.DRS(gtex, sigsBrain[[j]])
    }
  
  ## DTA
    for (j in names(sigsBrain)) {
    # for (j in names(sigsBrain)[grep("z", names(sigsBrain))]) {
      est$DTA[[j]] <- run.DTA(gtex, sigsBrain[[j]], alg = "diff", q = 0.01)
    }  
  ...
  for(j in names(sigsMuSiC)) {
      ...
      ## Deconvolution with merged subtypes
        ...
        gc()
        est$MUS[[j]] <- run.music(mixture = gtexCounts,
        ...
        gc()
        est$MUS[[paste0(j, ".")]] <- run.music(mixture = gtexCounts,
                                               use.meta.column = "brain.ct2",
        ...
        gc()
        est$MUS[[paste0(j, "_")]] <- run.music(mixture = gtexCounts, 
      ...
      #   if (j %in% c("TS", "DM")) next # skip these signatures
      #   
      #   if (j == "CA") {
      ...
      #   gc()
      #   est$MUS[[paste0(j, "z")]] <- run.music(mixture = gtexCounts, 
...
## Save
save(est, est.ctx, file = "Composition Estimates (Final) (New MUS).rda") # load("Composition Estimates (Final) (New MUS).rda")
...
y <- do.call("cbind", y)
write.csv(y, file = "ST2_EstimatedComposition_SourceFiles_GTEx.csv")
...
# ## What variables influence goodness of fit?
#   covar <- data.frame(Batch = as.factor(gtexMeta$SMGEBTCH), # expression batch ID
#                     RIN = as.numeric(gtexMeta$SMRIN), # rna integrity number
#                     PMI = as.numeric(gtexMeta$SMTSISCH), # post mortem interval
#                     Region = as.factor(gtexMeta$BroadRegion), # region of the brain
#                     Age = as.factor(gtexMeta$Age),
#                     Sex = as.factor(gtexMeta$Sex),
#                     np = as.numeric(est$CIB$SC.CA.IP$Neurons)) # neuronal proportion estimated by CIBERSORT/IP signature
#   
#   response <- gof$CIB$SC.CA.IP$r
#   
#   lm <- lm(response ~ RIN + PMI + Region + Age + Sex, data = covar)
#   sum <- summary(lm)
#   sum <- sum$coefficients  
#   
#   lm <- lm(response[ctx] ~ RIN + PMI + Age + Sex, data = covar[ctx,])
#   sum <- summary(lm)
#   sum <- sum$coefficients  
...
g <- grep("F5|xC|Coe|Bl|NG", colnames(cors))
h <- cors[-g,-g]
i <- h > 0.8
sum(apply(i, 1, function(x) length(which(x))))
