...
## Directory
  ...
  setwd(paste0(root.dir, "Results/BenchmarkingDatasets/CA/"))
...
## Modify signatures
  ## Modify the formatting of the VL signature
   ...
   ## Remove $CA from sigsMuSiC, as it's in sigsSNME
    sigsMuSiC <- sigsMuSiC[-grep("CA", names(sigsMuSiC))]
    
  ## Rename signatures
    sigsSNME[-which(names(sigsSNME) == "CA")] <- lapply(sigsSNME[-which(names(sigsSNME) == "CA")], function(x) {
...
true <- list()
true$orig.celltype <- as.data.frame(bench_CA$true$orig.celltype)    
...
## Deconvolve using the CA (matched) signatures
  ...
  for(k in names(sigsSNME$CA[4:6])) { # orig.celltype, merge1, and merge2
    
    # setup  
    est$CA[[k]] <- list()

    print(paste0(k, "_DRS"))
    est$CA[[k]]$DRS <- run.DRS(bench_CA$mixture$cpm, sigsSNME$CA[[k]])

    print(paste0(k, "_DTA"))
    est$CA[[k]]$DTA <- run.DTA(bench_CA$mixture$cpm, sigsSNME$CA[[k]], alg = "diff", q = 0.01)

    print(paste0(k, "_CIB"))
    est$CA[[k]]$CIB <- run.CIB(from.file = FALSE,
                                 sigObject = sigsSNME$CA[[k]],
                                 mixString = "bench_CA.txt")
    
    est$CA[[k]]$MUS <- run.music(mixture = bench_CA$mixture$counts, use.meta.column = k, drop.ct = FALSE,
...
run.deconv <- function(sig, subneurons = TRUE) {
    ...
    e$merge1 <- list()
    e$merge1$DRS <- run.DRS(bench_CA$mixture$cpm, s1)
    e$merge1$DTA <- run.DTA(bench_CA$mixture$cpm, s1, alg = "diff", q = 0.01)
    ...
    if (sig %in% names(sigsMuSiC)) {
      e$merge1$MuSiC <- run.music(mixture = bench_CA$mixture$counts, 
    ...
    if (subneurons) {
      e$merge2 <- list()
      e$merge2$DRS <- run.DRS(bench_CA$mixture$cpm, s2)
      e$merge2$DTA <- run.DTA(bench_CA$mixture$cpm, s2, alg = "diff", q = 0.01)
      print("Starting CIB merge2")
      e$merge2$CIB <- run.CIB(from.file = FALSE,
                              sigObject = s2,
                              mixString = "bench_CA.txt")
      if (sig %in% names(sigsMuSiC)) {
        e$merge2$MUS <- run.music(mixture = bench_CA$mixture$counts, 
...
## Enrichment
  enrich$xCell <- run.xCell(bench_CA$mixture$cpm)
  enrich$Blender <- run.Blender(bench_CA$mixture$cpm)
    

## Save
  save(est, enrich, file = "Final CA_Bench Composition.rda")
...
## A coex function
  analyse.coex <- function(coex, merge) {
    ...
    coex <- define.celltypes(x = coex, algorithm.type = "Coex", celltype.signature = sigsSNME$CA[[merge]], 
                             return.confidence = TRUE, nMarks.x = 10000, nMarks.sig = 100, bg = rownames(bench_CA$mixture$cpm))
...
## Linseed: on the standard mixtures
  complete$linseed <- apply.linseed(bench_CA$mixture$cpm, "merge2", "merge2")

## Linseed: on the gradient mixtures
  complete$linseed.grad <- apply.linseed(grad_CA$confound.p, "merge2", "merge2.grad")
    
  

## Coex: on the standard mixtures
  ## Run
    coex.standardOutput <- run.coex(mixture = bench_CA$mixture$cpm, 
                                    signature = sigsSNME$CA$merge2, 
...
## Coex: on the gradient mixtures
  ## Run
    coex.gradientOutput <- run.coex(mixture = grad_CA$confound.p,
                                      signature = sigsSNME$CA$merge2,
                                      only.threshold = FALSE,
                                      sft = "auto", 
                                      output.all = TRUE)
...
## Plot barplot
  plot.formal.barplot <- function(sig, merge, add.enrich.algs = TRUE, ncol = 1, lower.ylim = NA) {
    ...
    if (add.enrich.algs) {
      ...
      dat["OPCs", "Blender"] <- cor(enrich$Blender$OPC, true$merge1$OPC)
  ...
  plot.formal.barplot("CA", "merge1", add.enrich.algs = FALSE) #+ theme(axis.text.x = element_text(angle = 90, hjust = 1)) + NoLegend()
  dev.off()
  
  pdf(file = "Final Barplot - HCA, merge2.pdf", height = 6.38, width = 2)
  plot.formal.barplot("CA", "merge2", add.enrich.algs = FALSE) + theme(axis.text.y = element_blank(), axis.title.y = element_blank(), axis.ticks.y = element_blank()) 
  dev.off()
  
  
  pdf(file = "Final Barplot - HCA, All Clusters.pdf", height = 7.07, width = 3.4)
  plot.formal.barplot("CA", "orig.celltype", add.enrich.algs = FALSE)  + theme(axis.text.y = element_blank(), axis.title.y = element_blank(), axis.ticks.y = element_blank()) 
  dev.off()
...
## Goodness of fit across signatures!
  ...
  get.gof <- function(e, sig) {
    sig <- sig[,colnames(e)]
    output <- write.gof.v2(estimatedComp = e, measuredExp = bench_CA$mixture$cpm, signatureUsed = sig)
    output <- output$r
  }
