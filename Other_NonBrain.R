## This script looks at the effect of algorithms and signatures in two non-brain tissues: heart and pancreas.
  # whereas the core analyses use separate scripts for processing and analysis, this script does both


## Note: within Data/Raw, you need a directory called "ENCODE_PrimaryCells", and place the files ENCODE_Metadata.csv and all files downloaded from ENCODE_files.txt for this script to run
## Note that ENCODE_Metadata.csv is just a CSV version of the first file listed in ENCODE_files.txt
...
## Directory
  ...
  wd2 <- paste0(root.dir, "Results/Other/NonBrain/") # and for analysis
  setwd(wd1)
...
## Load FANTOM5 data
  F5.exp <- read.csv("Raw/Fantom5Consortium_CAGE_Filtered.csv", row.names = 1) 
...
## Load ENCODE metadata
  encode.meta <- read.csv("Raw/ENCODE_PrimaryCells/ENCODE_metadata.csv", header = TRUE) # note: I converted the tsv to a csv in excel
...
## Prepare single-cell signature from Wang et al. 2020
  heart.sc <- read.csv("Raw/GSE109816_normal_heart_umi_matrix", row.names = 1)
  heart.clust <- read.table("Raw/GSE109816_normal_heart_cell_cluster_info.txt", sep = "\t", header = 1)
